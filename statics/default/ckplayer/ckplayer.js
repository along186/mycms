/*
JavaScript Document
ckplayer5.8,有问题请访问http://www.ckplayer.com,欢迎把您的风格提供给大家分享
*/
...
function ckstyle() { //定义总的风格
	...
	ck.control_c='Null,1,1,-180,-100,ckplayer/share.xml?id=[$pat],0';//分享按钮的文件，坐标,相关设置的xml文件,编码0是默认的utf-8,1是gbk2312
	ck.control_c2='Null,1,1,-180,-100';//调整大小和颜色按钮的文件，坐标
	ck.control_rel='Null,ckplayer/related.xml?id=[$pat],0';
	...
	ck.setup = '0,1,1,1,1,1,1,0';
    //总体设置：是否使用鼠标手型1是使用手型0是不使用，是否支持单击暂停1是支持0不支持，是否支持双击全屏1是支持0不支持,在播放前置flash广告时是否同时加载视频,logo在播放广告时是否显示1不显示0显示,是否采用http视频直播流0不是1采用,,是否在隐藏控制栏时调用视频大小0不是1是,是否使用视频拉伸0默认1拉伸
    ck.pm_repc='[]->&';
    ...
    ck.pm_play = '0,2,0,-30';
    //播放和暂停按钮的坐标
    ck.pm_clock = '0,2,40,0';
    //已播放时间的坐标和宽度
    ck.pm_clock2 = '0,2,40,-25';
    //总时间的坐标和宽度
...
function ckplayer_hqurl(str){var webagreement=document.location.protocol;var dqljurl=unescape(window.location.href).replace("file:///","");var port=document.location.port;var hostname=document.location.hostname;if(port){hostname+=':'+port}var dqweburl=document.location.protocol+'//'+hostname;var lagree='';var iswb=0;var geturl='';var geturlt='';var strs=new Array();strs=str.split('//');if(strs.length>0){lagree=strs[0]+'//'}var httpgreenall="http|https|ftp|rtsp|mms|ftp|rtmp";var httparray=new Array();httparray=httpgreenall.split('|');for(i=0;i<httparray.length;i++){if((httparray[i]+'://')==lagree){iswb=1;break}}if(iswb==0){if(str.substr(0,1)=='/'){geturlt=dqweburl+str}else{geturl=dqljurl.substring(0,dqljurl.lastIndexOf("/")+1).replace("\\","/");strt=str.replace('../','./');var str2=new Array();str2=strt.split('./');var str2length=str2.length;var strs=strt.replace('./','');var urlt=new Array();urlt=geturl.split('/');var jqcd=urlt.length-str2length;for(i=0;i<(jqcd);i++){geturlt+=urlt[i]+'/'}geturlt+=strs}}else{geturlt=str}return geturlt}
function swfupload() {
    this.ckplayer_url = '/ckplayer/ckplayer.swf';
    this.ckplayer_flv = '';
    this.ckplayer_pat = '';
    this.ckplayer_style = 0;
	...
	this.ckplayer_prompt='';
	this.ckplayer_prompttext='';
