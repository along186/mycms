...
define('IS_CGI',substr(PHP_SAPI, 0,3)=='cgi' ? 1 : 0 );  //用来判断是使用命令行还是浏览器执行的，如果 PHP_SAPI==’cli’ 表示是在命令行下执行
define('IS_WIN',strstr(PHP_OS, 'WIN') ? 1 : 0 );         //用来判断服务器的操作系统值为（WINNT，WIN32等）
...
function load_runtime_file() {
    ...
    foreach ($list as $key=>$file){
        if(is_file($file))  require_cache($file);  //防止重复引用
    }
    // 加载系统类库别名定义
