...
class ContentAction extends AdminbaseAction {
    ...
    public function get_xingbe_list()
    {
       $CateTable = M("category");
       $arrrange = $CateTable->where(array('parentid'=>389))->order('listorder asc')->select();
       $arrlist = array();
       foreach ($arrrange as $key=>$row)
       {
           $arrlist[$key][v] = "x".$row['catid'];
           $arrlist[$key][n] = $row['catname'];  
       }
       echo json_encode($arrlist);
    }
    public function get_diqu_list()
    {
       $CateTable = M("category");
       $arrrange = $CateTable->where(array('parentid'=>392))->order('listorder asc')->select();
       $arrlist = array();
       foreach ($arrrange as $key=>$row)
       {
           $arrlist[$key][v] = "d".$row['catid'];
           $arrlist[$key][n] = $row['catname'];  
       }
       echo json_encode($arrlist);
    }
    public function get_leixing_list()
    {
       $CateTable = M("category");
       $arrrange = $CateTable->where(array('parentid'=>398))->order('listorder asc')->select();
       $arrlist = array();
       foreach ($arrrange as $key=>$row)
       {
          $arrlist[$key][v] = "l".$row['catid'];
          $arrlist[$key][n] = $row['catname'];  
       }
       echo json_encode($arrlist);
    }
    public function get_ccf_list(){
        $CateTable = M("category");
        $arrrange = $CateTable->where(array('parentid'=>427))->order('catid asc')->select();
        $arrlist = array();
        foreach ($arrrange as $key=>$row)
        {
            $arrlist[$key][v] = "c".$row['catid'];
            $arrlist[$key][n] = $row['catname'];
        }
        echo json_encode($arrlist);
    }
    public function get_dyf_list(){
        $CateTable = M("category");
        $arrrange = $CateTable->where(array('parentid'=>436))->order('catid asc')->select();
        $arrlist = array();
        foreach ($arrrange as $key=>$row)
        {
            $arrlist[$key][v] = "d".$row['catid'];
