...
class ModelModel extends CommonModel {
    ...
    const mainTableSql = 'Sql/Longcms_zhubiao.sql'; //模型主表SQL模板文件
    const sideTablesSql = 'Sql/Longcms_zhubiao_data.sql'; //模型副表SQL模板文件
    const modelTablesInsert = 'Sql/Longcms_insert.sql'; //可用默认模型字段
    const membershipModelSql = 'Sql/Longcms_member.sql'; //会员模型
    ...
    public function AddModelMember($tableName, $modelId) {
        ...
        //表前缀，表名，模型id替换
        $sqlSplit = str_replace(array('@IITcms@', '@zhubiao@', '@modelid@'), array($dbPrefix, $tableName, $modelId), $membershipModelSql);
    ...
    protected function createModel($tableName, $modelId) {
        ...
        //表前缀，表名，模型id替换
        $sqlSplit = str_replace(array('@Longcms@', '@zhubiao@', '@modelid@'), array($dbPrefix, $tableName, $modelId), $mainTableSqll . "\n" . $sideTablesSql . "\n" . $modelTablesInsert);
    ...
    public function sql_split($sql) {
        ...
        if (C("DB_PREFIX") != "IITcms_") {
            $sql = str_replace("IITcms_", C("DB_PREFIX"), $sql);
        }
        $sql = str_replace("\r", "\n", $sql);
        $ret = array();
        $num = 0;
