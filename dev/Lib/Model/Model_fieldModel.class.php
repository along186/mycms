...
class Model_fieldModel extends CommonModel {
    ...
    protected $_validate = array(
        ...
        array('field', '/^[a-z]+$/i', '字段名只支持英文！', 0, 'regex', 3),
    ...
    public function addField($data) {
        ...
        if ($data) {
             ...
             */
            require C("LONG_FIELDS_PATH") . $data['formtype'] . "/config.inc.php";
    ...
    public function editField($data, $fieldid = 0) {
        ...
        /**
         * 载入字段配置
         * $not_allow_fields 不显示的字段类型（字段类型）
         * $unique_fields 允许添加但必须唯一的字段（字段名）
         * $forbid_fields 禁止被禁用的字段列表（字段名）
         * $forbid_delete 禁止被删除的字段列表（字段名）
         * $att_css_js 可以追加 JS和CSS 的字段（字段名）
         */
        require C("LONG_FIELDS_PATH") . "fields.inc.php";
        /**
         * 对应字段配置
         * $field_type = 'varchar'; //字段数据库类型
         * $field_basic_table = 1; //是否允许作为主表字段
