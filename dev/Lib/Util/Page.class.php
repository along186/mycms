...
class Page {
    ...
    function __construct($Total_Size = 1, $Page_Size = 20, $Current_Page = 1, $List_Page = 6, $PageParam = 'page', $pageRule = '', $static = FALSE, $static_size = 0) {
        //默认模板配置
        $this->Page_tpl['default'] = array('Tpl' => '{first}{prev}&nbsp;{liststart}{list}{listend}&nbsp;{next}{last}', 'Config' => array());
        //每页显示信息数量
    ...
    protected function Pager($Page_tpl = '') {
        ...
        for ($i = $pStart; $i <= $pEnd; $i++) {
            ...
            //此处不分静态动态页面
            if ($i == $cfg ['pageindex']) {
                $pList .= '<a href="javascript:void(0);" class="cur">' . str_replace('*', $i, $cfg['list']) . '</a> ';
            } else {
