...
class ContentTagLib {
    ...
    public function where($attr) {
        ...
        if (isset($attr['chu']) && !empty($attr['chu'])) {
        	 $where['ccf'] = array("EQ", $attr['chu']);  		
        }
        if (isset($attr['dai']) && !empty($attr['dai'])) {
             $where['dyf'] = array("EQ", $attr['dai']);
        }
        if (isset($attr['sex']) && !empty($attr['sex'])) {
        	$where['xingbe'] = array("EQ", $attr['sex']);   
        }
        if (isset($attr['diqu']) && !empty($attr['diqu'])) {
            $where['diqu'] = array("EQ", $attr['diqu']);
        }
        if (isset($attr['zhiye']) && !empty($attr['zhiye'])) {
            $where['leixing'] = array("EQ", $attr['zhiye']);
        }
        if (isset($attr['starname']) && !empty($attr['starname'])) {
            $where['title'] = array("LIKE", $attr['starname']);
        }
        //缩略图
        if (isset($attr['thumb'])) {
            if ($attr['thumb']) {
    ...
    public function article($data) {
        ...
        $artid = intval($data['artid']);
        $model=new Model();
        if (!empty($artid))
        {
           $data = $model->table(C("DB_PREFIX").$table.' a,'.C("DB_PREFIX").$table.'_data d')->where('a.id=d.id and a.id='.$artid)->find();
        }
        else 
        {
           $data = $model->table(C("DB_PREFIX").$table.' a,'.C("DB_PREFIX").$table.'_data d')->where('a.id=d.id and a.catid='.$data['catid'])->order('updatetime desc')->limit('1')->find();
