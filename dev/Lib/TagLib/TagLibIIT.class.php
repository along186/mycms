 ...
 */
class TagLibIIT extends TagLib {

    // 数据库where表达式
    protected $comparisonIITcms = array(
        '{notlike}' => 'NOT LIKE',
        '{notin}' => 'NOT IN',
        '{like}' => 'LIKE',
        '{eq}' => '=',
    ...
    protected $tags = array(
        //内容标签
        'content' => array('attr' => 'action,cache,num,page,pagetp,pagefun,return,where,moreinfo,thumb,order,day,catid,table,chu,dai,sex,diqu,zhiye,starname', 'level' => 3),
        //Tags标签
     ...
     * 		@where		sql语句的where部分
     * #当action为article时，调用文章
     * 	#用法示例：<content action="article"  artid="$artid" table="$table"> .. HTML ..</content>
     * 	独有参数：
     * 		@artid		文章ID
     ...
     * 标签：<get></get>
     * 作用：特殊标签，SQL查询标签
     * 用法示例：<get sql="SELECT * FROM IITcms_article  WHERE status=99 ORDER BY inputtime DESC" page="$page" num="5"> .. HTML ..</get>
     * 参数说明：
    ...
    public function _get($attr, $content) {
        ...
        $tag['sql'] = $sql = str_replace(array("think_", "IITcms_"), C("DB_PREFIX"), strtolower($tag['sql']));
        //数据源
        ...
        if ($page) {
            ...
            if ($_sql = preg_replace('/select([^from].*)from/i', "SELECT COUNT(*) as count FROM ", $tag['sql'])) {
                ...
                if (substr(trim($sql), 0, 1) == '$') {
                    $parseStr .= ' $sql = str_replace(array("think_", "IITcms_"), C("DB_PREFIX"),' . $sql . ');';
            ...
            //判断是否变量传递
            if (substr(trim($sql), 0, 1) == '$') {
                $parseStr .= ' $_sql = str_replace(array("think_", "IITcms_"), C("DB_PREFIX"),' . $sql . ');';
            } else {
                $parseStr .= ' $_sql = "' . str_replace('"', '\"', $sql) . '";';
            }
            $parseStr .= ' $cache = ' . $cache . ';';
    ...
    protected function parseSqlCondition($condition) {
        $condition = str_ireplace(array_keys($this->comparisonIITcms), array_values($this->comparisonIITcms), $condition);
        return $condition;
    }
